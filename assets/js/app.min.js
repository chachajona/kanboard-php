...
var KB = {
	...
	html: {},
	http: {},
	listeners: { clicks: {}, changes: {}, keys: [], internals: {} },
};
...
KB.listen = function () {
	...
	function onClick(e) {
		for (var selector in self.listeners.clicks) {
			if (
				self.listeners.clicks.hasOwnProperty(selector) &&
	...
	function onChange(e) {
		for (var selector in self.listeners.changes) {
			if (
				self.listeners.changes.hasOwnProperty(selector) &&
				e.target.matches(selector)
			) {
	...
	function onKeyPressed(e) {
		...
		if (keysQueue.length > 0) {
			...
			for (var i = 0; i < self.listeners.keys.length; i++) {
				...
				if (
					KB.utils.arraysIdentical(keysQueue, sequence) &&
					...
					KB.utils.arraysStartsWith(keysQueue, sequence) &&
					keysQueue.length < sequence.length
				) {
					reset = !1;
				}
			}
...
(function () {
	...
	function onOverlayClick(e) {
		if (e.target.matches("#modal-overlay") && isFormDirty === !1) {
			e.stopPropagation();
			e.preventDefault();
			destroy();
		}
	...
	function getForm() {
		return document.querySelector(
			"#modal-content form:not(.js-modal-ignore-form)"
		);
	}
	function submitForm() {
		var form = getForm();
	...
	function afterRendering() {
		...
		if (formElement) {
			formElement.on("change", onFormChange, !1);
			formElement.on("submit", onFormSubmit, !1);
		}
		var autoFocusElement = document.querySelector(
			"#modal-content input[autofocus]"
		);
		if (autoFocusElement) {
	...
	function replace(html) {
		...
		if (contentElement) {
			contentElement.replace(
				KB.dom("div").attr("id", "modal-content").html(html).build()
			);
			afterRendering();
		}
	}
...
(function () {
	function redirectToTaskView(e) {
		var ignoreParentElement = KB.dom(e.target).parent(
			"a, .task-board-change-assignee"
		);
...
KB.component(
	...
	function (containerElement, options) {
		this.render = function () {
			...
			c3.generate({
				data: { columns: [plots], type: "bar" },
				bar: { width: { ratio: 0.5 } },
				axis: {
					x: { type: "category", categories: categories },
					y: { tick: { format: KB.utils.formatDuration } },
				},
				legend: { show: !1 },
			});
		};
...
KB.component(
	"chart-project-estimated-actual-column",
	function (containerElement, options) {
		this.render = function () {
			...
			KB.dom(containerElement).add(KB.dom("div").attr("id", "chart").build());
			c3.generate({
				data: { columns: [spent, estimated], type: "bar" },
				bar: { width: { ratio: 0.2 } },
				axis: { x: { type: "category", categories: columns } },
				legend: { show: !0 },
			});
		};
	}
...
KB.component(
	...
	function (containerElement, options) {
		this.render = function () {
			...
			KB.dom(containerElement).add(KB.dom("div").attr("id", "chart").build());
			c3.generate({
				data: { columns: [lead, cycle], types: types, colors: colors },
				axis: {
					x: { type: "category", categories: categories },
					y: { tick: { format: KB.utils.formatDuration } },
				},
...
KB.component(
	"chart-project-task-distribution",
	function (containerElement, options) {
		this.render = function () {
			var columns = [];
			for (var i = 0; i < options.metrics.length; i++) {
				columns.push([
					options.metrics[i].column_title,
					options.metrics[i].nb_tasks,
				]);
			}
			KB.dom(containerElement).add(KB.dom("div").attr("id", "chart").build());
			c3.generate({ data: { columns: columns, type: "donut" } });
		};
	}
);
KB.component(
	...
	function (containerElement, options) {
		this.render = function () {
			...
			for (var status in options.metrics) {
				...
				categories.push(
					status === "open" ? options.labelOpen : options.labelClosed
				);
			}
			KB.dom(containerElement).add(KB.dom("div").attr("id", "chart").build());
			c3.generate({
...
KB.component(
	"chart-project-user-distribution",
	function (containerElement, options) {
		this.render = function () {
			var columns = [];
			for (var i = 0; i < options.metrics.length; i++) {
...
KB.component("chart-task-time-column", function (containerElement, options) {
	this.render = function () {
		...
		KB.dom(containerElement).add(
			KB.dom("div").attr("id", "chart-task-time-column").build()
		);
		c3.generate({
...
KB.component("external-task-view", function (containerElement, options) {
	this.render = function () {
		KB.dom(containerElement).html(
			'<div id="external-task-view"><i class="fa fa-spinner fa-2x fa-pulse"></div>'
		);
		$.ajax({
			cache: !1,
			url: options.url,
			success: function (data) {
				KB.dom(containerElement).html(
					'<div id="external-task-view">' + data + "</div>"
				);
			},
		});
	};
...
KB.component("file-upload", function (containerElement, options) {
	...
	function onProgress(e) {
		if (e.lengthComputable) {
			...
			KB.find("#file-percentage-" + currentFileIndex).replaceText(
				"(" + percentage + "%)"
			);
		}
	}
	...
	function onRequestTooLarge() {
		var label = options.labelOversize
			? options.labelOversize
			: options.labelUploadError;
	...
	function onComplete() {
		...
		if (currentFileIndex < files.length) {
			...
			var buttonElement = KB.dom("button")
				...
				.build();
			KB.dom(dropzoneElement).replace(
				KB.dom("div").add(alertElement).add(buttonElement).build()
			);
		}
	}
	...
	function buildFileListItem(index) {
		...
		if (options.maxSize > 0 && files[index].size > options.maxSize) {
			itemElement.add(
				KB.dom("div").addClass("file-error").text(options.labelOversize).build()
			);
			isOversize = !0;
		}
		if (isOversize) {
...
KB.component("image-slideshow", function (containerElement, options) {
	...
	function renderSlide() {
		...
		var previousElement = KB.dom("div")
			.attr(
				"class",
				"fa fa-chevron-circle-left slideshow-icon slideshow-previous-icon"
			)
			.build();
		var nextElement = KB.dom("div")
			.attr(
				"class",
				"fa fa-chevron-circle-right slideshow-icon slideshow-next-icon"
			)
			.build();
...
KB.keyboardShortcuts = function () {
	...
	KB.onKey("?", function () {
		if (!KB.modal.isOpen()) {
			KB.modal.open(KB.find("body").data("keyboardShortcutUrl"), "", !0);
		}
	});
	KB.onKey(
		"Escape",
...
document.addEventListener("DOMContentLoaded", function () {
	...
	function onItemChange(event) {
		var selectedItems = document.querySelectorAll(
			"input[data-list-item=selectable]:checked"
		);
	...
	KB.on("dropdown.afterRender", function () {
		var actionLinks = document.querySelectorAll(
			"a[data-list-item-action=modal]"
		);
...
(function () {
	...
	KB.onClick(".js-modal-confirm", function (e) {
		KB.modal.open(getLink(e), "small", !0);
	});
...
KB.component("project-select-role", function (containerElement, options) {
	...
	function updateRole() {
		KB.http
			.postJson(options.url, { id: options.id, role: options.role })
			.success(function () {
				isLoading = !1;
				isSuccess = !0;
				replaceComponentElement();
	...
	function buildComponentElement() {
		...
		if (isLoading) {
			container.text(" ");
			container.add(
				KB.dom("i").attr("class", "fa fa-spinner fa-pulse fa-fw").build()
			);
		} else if (isSuccess) {
			container.text(" ");
			container.add(
				KB.dom("i")
					.attr("class", "fa fa-check fa-fw icon-fade-out icon-success")
					.build()
			);
		} else if (isError) {
			container.text(" ");
			container.add(
				KB.dom("i")
					.attr("class", "fa fa-check fa-fw icon-fade-out icon-error")
					.build()
			);
		}
	...
	function (containerElement, options) {
		...
		function onScroll() {
			...
			if (menuElement) {
				...
				menuElement.style(
					"top",
					document.body.scrollTop + componentPosition.bottom + "px"
				);
			}
		}
		function onKeyDown(e) {
		...
		function insertSelectedItem() {
			...
			destroyDropdownMenu();
			if (options.redirect) {
				window.location = options.redirect.url.replace(
					new RegExp(options.redirect.regex, "g"),
					value
				);
			} else if (options.replace) {
				onLoadingStart();
				KB.modal.replace(
					options.replace.url.replace(
						new RegExp(options.replace.regex, "g"),
						value
					)
				);
			}
		...
		function filterItems(text, items) {
			...
			for (var i = 0; i < items.length; i++) {
				if (
					text.length === 0 ||
					items[i]["data-label"].toLowerCase().indexOf(text.toLowerCase()) > -1
				) {
					var item = items[i];
					if (
						typeof options.defaultValue !== "undefined" &&
						String(options.defaultValue) === item["data-value"]
					) {
						item.class += " active";
						hasActiveItem = !0;
					}
...
(function () {
	function savePosition(subtaskId, position) {
		...
		$.ajax({
			...
			type: "POST",
			processData: !1,
			data: JSON.stringify({ subtask_id: subtaskId, position: position }),
		});
	}
	function bootstrap() {
...
KB.component("task-move-position", function (containerElement, options) {
	...
	function onError(message) {
		KB.trigger("modal.stop");
		KB.find("#message-container").replace(
			KB.dom("div")
	...
	function onSubmit() {
		...
		KB.find("#message-container").replace(
			KB.dom("div").attr("id", "message-container").build()
		);
		KB.http
			.postJson(options.saveUrl, {
...
KB.component("text-editor", function (containerElement, options) {
			...
			.for("a", [
				{
					href: "#",
					html:
						'<i class="fa fa-pencil-square-o fa-fw"></i> ' + options.labelWrite,
					click: function () {
						toggleViewMode();
					},
				},
	...
	function insertLinkTag() {
		...
		textarea.setSelectionRange(
			currentSelectionStart + selectionStartOffset,
			currentSelectionStart + selectionEndOffset
		);
	}
	function insertText(replacedText) {
		...
		if (!result) {
			...
			textarea.value = replaceTextRange(
				textarea.value,
				selectionStart,
				selectionEnd,
				replacedText
...
Kanboard.App.prototype.tagAutoComplete = function () {
	$(".tag-autocomplete").select2({ tags: !0 });
};
...
Kanboard.BoardColumnView.prototype.showColumn = function (columnId) {
	...
	$(".board-column-header-" + columnId).removeClass(
		"board-column-header-collapsed"
	);
...
Kanboard.BoardHorizontalScrolling.prototype.render = function () {
	if (localStorage.getItem("horizontal_scroll") == 0) {
		...
		$("#board th:not(.board-column-header-collapsed)").addClass(
...
Kanboard.BoardPolling.prototype.check = function () {
	if (
		KB.utils.isVisible() &&
		!this.app.get("BoardDragAndDrop").savingInProgress
	) {
		var self = this;
		this.app.showLoadingIcon();
		$.ajax({
...
Kanboard.Column.prototype.savePosition = function (columnId, position) {
	...
	$.ajax({
		...
		type: "POST",
		processData: !1,
		data: JSON.stringify({ column_id: columnId, position: position }),
		complete: function () {
			self.app.hideLoadingIcon();
		},
	});
...
Kanboard.Dropdown.prototype.listen = function () {
	...
	$(document).on("click", ".dropdown-menu", function (e) {
		...
		if (
			offset.top + submenuHeight - $(window).scrollTop() < $(window).height() ||
			$(window).scrollTop() + offset.top < submenuHeight
		) {
...
Kanboard.Search.prototype.listen = function () {
	$(document).on("click", ".filter-helper", function (e) {
		...
		if (uniqueFilter) {
			...
			filter = input
				.val()
				.replace(new RegExp("(" + attribute + ":[#a-z0-9]+)", "g"), "");
...
Kanboard.Swimlane.prototype.collapse = function (swimlaneId) {
	...
	$(".board-swimlane-columns-" + swimlaneId + ":not(:first-child)").css(
		"display",
		"none"
	);
...
Kanboard.Swimlane.prototype.savePosition = function (swimlaneId, position) {
	...
	$.ajax({
		...
		type: "POST",
		processData: !1,
		data: JSON.stringify({ swimlane_id: swimlaneId, position: position }),
		complete: function () {
			self.app.hideLoadingIcon();
		},
	});
	...
	function renderColorOption(color) {
		return $(
			'<div class="color-picker-option">' +
				'<div class="color-picker-square color-' +
				color.id +
				'"></div>' +
...
Kanboard.BoardDragAndDrop.prototype.save = function (
	taskId,
	srcColumnId,
	dstColumnId,
	position,
